...
统一写入接口是主修复手段，但**仍然需要互斥保护**。

原因是如果两个流程并发进入统一接口，仍然可能发生竞态：

1. 流程 A 查询 `inbound + email`，结果为空
2. 流程 B 查询 `inbound + email`，结果也为空
3. 流程 A 执行 `addClient()`
4. 流程 B 也执行 `addClient()`
5. 最终仍可能写出重复 email
...
- 并发情况下减少竞态窗口
- 避免两个流程同时对同一个 `inbound + email` 做新增/删除/更新
...
- 如需要，也可补充 `server/db/` 下的锁辅助封装

建议在 `upsertUniqueClient(...)` 最外层增加互斥，锁粒度为：

- `server_id + inbound_id + email`

原因：

1. 真实唯一约束就是“同一 inbound 下 email 唯一”
2. 不同 inbound 下同 email 合法，不应该互相阻塞
3. 锁粒度比“按用户 ID 加锁”更准确

锁的层级建议如下：

1. **首选：PostgreSQL advisory lock**
   - 使用数据库级锁覆盖支付同步、定时任务、重试任务、未来多实例
   - 在进入 `upsertUniqueClient(...)` 前获取锁，退出时释放

2. **可选补充：进程内锁**
   - 用于减少同一 Node 进程内的重复进入
   - 但它不能替代数据库锁

本设计建议明确采用：

- **统一唯一化写入接口 + 数据库级互斥锁** 的双保险方案
...
如果获取数据库级锁失败，也应视为本次同步失败或等待重试，不能在未持锁状态下继续执行唯一化写入。
...
调用方（支付同步 / 定时同步 / 重试任务）
  ...
  -> upsertUniqueClient(inbound, email, desiredClient, context)
     -> 获取 server_id + inbound_id + email 级别互斥锁
     -> getClientsByEmail(inboundId, email)
     -> 匹配数量 = 0
        -> addClient
     -> 匹配数量 = 1
...
6. **两个并发流程同时同步同一 `server + inbound + email`**
   - 期望：只有一个流程进入真实写入阶段，另一个等待或失败重试
...
3. 支付同步、定时同步、失败重试使用同一套唯一化写入逻辑，并共享同一套互斥保护
